---
name: design-tokens
description: Create W3C-compliant design token JSON files and explain W3C DTCG specification concepts. Use when writing tokens.json, editing design token definitions, or answering questions about how design tokens work (aliases, references, resolvers, token types, color spaces, etc.). Covers both practical implementation and conceptual understanding of the W3C Design Tokens specification.
---

# W3C Design Tokens Skill

This skill helps you write design token JSON that conforms to the W3C Design Token Community Group (DTCG) specification and understand how the specification works conceptually. The focus is on **correct JSON structure, type compliance, and deep understanding of specification concepts**.
...
When creating or modifying design token files, you MUST follow this workflow:

1. Create or edit the token/resolver JSON file
   - **When creating a NEW file**, always include a `$schema` property at the top:
     - For token files: `"$schema": "https://designtokens.org/schemas/2025.10/format.json"`
     - For resolver files: `"$schema": "https://designtokens.org/schemas/2025.10/resolver.json"`
2. **IMMEDIATELY validate** using the bundled script:
...
3. If validation fails, fix errors before proceeding (see [Fixing Validation Errors](#fixing-validation-errors))
4. Only consider the task complete when validation passes

**Do not skip validation.** Invalid token files will cause downstream tool failures.
...
- **Answering conceptual questions** about how design tokens work (e.g., "What's the difference between curly brace aliasing and JSON pointer references?", "How do resolvers merge token sets?", "Why does the spec use structured color objects instead of hex strings?")
- **Explaining specification details** such as token inheritance, type system design, or resolution logic
- **Comparing approaches** within the design token ecosystem (e.g., aliasing strategies, color space choices, composite vs simple types)

**Important**: When answering conceptual questions, always reference the linked specification files (in the `spec/` directory) to provide accurate, in-depth explanations grounded in the official W3C DTCG specification.
...
After creating or modifying token files, validate them against the W3C DTCG schema using the bundled validation script:
...
**Always validate token files after creating or modifying them** to ensure W3C DTCG compliance.

### Fixing Validation Errors

When validation fails, use the appropriate resource based on the error type:

| Error Type                                              | Resource                                                         | When to Use                                                     |
| ------------------------------------------------------- | ---------------------------------------------------------------- | --------------------------------------------------------------- |
| Structural errors (wrong type, missing required fields) | JSON schemas in `schemas/format.json` or `schemas/resolver.json` | The schema shows exact type definitions and required properties |
| Semantic errors (wrong values, incorrect patterns)      | Spec files in `spec/` directory                                  | The spec explains concepts with examples and context            |

**Common fixes:**

- "TYPE must be object" → Check the schema for the expected object structure
- "Missing required property" → Schema lists all required fields for each type
- "Invalid enum value" → Schema defines allowed values (e.g., color spaces, units)

## Extensions

Use `$extensions` for vendor-specific data with reverse domain notation (e.g., `"com.figma": { "variableId": "..." }`). Extensions are tool-specific and ignored by tools that don't recognize them.

## Additional Resources

**JSON Schemas** (for validation and fixing structural errors):

- **Token format schema**: [schemas/format.json](schemas/format.json)
- **Resolver schema**: [schemas/resolver.json](schemas/resolver.json)

**Specification files** (for understanding concepts and seeing examples):
