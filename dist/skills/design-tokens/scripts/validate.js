      ...
      compileAsync(schema, meta) {
        ...
        async function _loadSchema(ref2) {
          ...
          if (p)
            return p;
          try {
            return await (this._loading[ref2] = loadSchema2(ref2));
          } finally {
...
async function loadReferencedSchemas(ajv2, schema, baseDir) {
  const processValue = async (value) => {
    ...
    if (typeof obj.$ref === "string" && !obj.$ref.startsWith("#") && !obj.$ref.includes("#")) {
      ...
      if (!schemaCache.has(refPath)) {
        ...
        if (!ajv2.getSchema(schemaId)) {
          ajv2.addSchema(refSchema, obj.$ref);
        }
        await loadReferencedSchemas(ajv2, refSchema, dirname(refPath));
      }
    }
...
async function validate(type2, file) {
  const schemaPath = SCHEMA_PATHS[type2];
  const schema = await loadSchema(schemaPath);
  const ajv2 = createValidator();
  await loadReferencedSchemas(ajv2, schema, dirname(schemaPath));
...
async function main() {
  const type2 = process.argv[2];
  const file = process.argv[3];
  const validTypes = ["tokens", "resolver"];
  if (!type2 || !validTypes.includes(type2) || !file) {
  ...
  const valid = await validate(type2, file);
